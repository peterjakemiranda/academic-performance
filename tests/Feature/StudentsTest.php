...
class StudentsTest extends TestCase
{
    ...
    public function test_can_search_for_students()
    {
        $this->actingAs($this->user)
    ...
    public function test_cannot_view_deleted_students()
    {
        ...
        $this->actingAs($this->user)
            ...
            ->assertInertia(fn (Assert $assert) => $assert
                ->component('Students/Index')
                ->has('students.data', 1)
                ->where('students.data.0.name', 'Lynn Kub')
            );
    }

    public function test_can_filter_to_view_deleted_students()
    {
        $this->user->account->students()->firstWhere('first_name', 'Martin')->delete();
        ...
        $this->actingAs($this->user)
            ...
            ->assertInertia(fn (Assert $assert) => $assert
                ->component('Students/Index')
                ->has('students.data', 2)
                ->where('students.data.0.name', 'Martin Abbott')
                ->where('students.data.1.name', 'Lynn Kub')
            );
    }
}
